...
def retrievedata(): #โหลดข้อมูลตอนเปิด
    ...
    tree_data = []
    try:
        with open("binarytree.txt", "r", encoding="utf-8") as file:
            for f in file:
                tree_box.insert(END, f.strip())
                tree_data.append(f.strip())
                print(tree_data)
    except:pass
...
def quit_save(): #ดำเนินการเมื่อคุณคลิกปุ่มออกและบันทึก
    ...
    with open("binarytree.txt", "w", encoding="utf-8") as file:
        for d in tree_data:
            file.write( d + "\n")
...
bquit = Button(treeroot, text="ออก และ บันทึก",font=1, command=quit_save).place(x=160,y=480)
